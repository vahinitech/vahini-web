...
- `analyser/` is a **git submodule** pinned to a vahinitech/20factor-analyser
  ...
  `input-manifest.yaml` (repo root) for the pinned repo/branch/commit/version,
  what's actually consumed (Docker image, proxied routes, this contract) and
  the bump procedure — CI fails if the manifest drifts from the real pin, so
  update it in the same commit as any pin bump.
