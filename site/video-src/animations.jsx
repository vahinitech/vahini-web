...
function Stage({
  ...
  // Toggle play/pause. If we're paused at the very end of a non-looping
  // timeline, pressing play should REPLAY from the start (otherwise the
  // animation loop would immediately hit the end and stop again).
  const togglePlay = React.useCallback(() => {
    setPlaying(p => {
      if (!p && !loop && time >= duration - 1e-3) { setTime(0); }
      return !p;
    });
  }, [loop, time, duration]);
