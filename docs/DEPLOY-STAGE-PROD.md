...
- Host-persisted Paddle model cache is stored at `$VAHINI_DATA_ROOT/paddle-models/.paddlex` (the data root defaults to the deploying user's home; `release.sh` and `prewarm-models.sh` export it).
- **Both compose files declare an explicit `name:`** (`vahini-stage` / `vahini-prod`), so Compose never falls back to deriving the project name from the invoking directory. Before this was added (incident: 2026-07-20), stage and prod shared the directory-derived default project name and identical service keys (`web`/`persist`/`analyser`); running `release.sh` for one silently recreated -- i.e. destroyed -- the other's containers via `up --remove-orphans`. If you ever add a third compose file for this repo, give it its own `name:` too.
- **One-time follow-up needed on the next real `release.sh prod` run** after pulling this fix: the currently-running `vahini-*-prod` containers were created before `name: vahini-prod` existed, so they're still labeled under the old default project. `docker stop vahini-site-prod vahini-persist-prod vahini-analyser-prod && docker rm vahini-site-prod vahini-persist-prod vahini-analyser-prod` right before that next `release.sh prod` run lets Compose recreate them cleanly under the correct project name instead of hitting a `container_name` conflict. Not urgent -- do it as part of the next planned prod deploy, not as a standalone action.